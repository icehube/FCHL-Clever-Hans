...
- [2026-08-05] [owner] state.py:158 — a team cannot bid its own `physical_max_bid` because of float error: `remaining_budget` computes to 4.199999999999996 (`56.8 - 52.6`), so the MILP's budget constraint rejects a forced bid at exactly 4.2 and `compute_marginal_value` returns one increment low (4.1). Costs $0.1M of headroom at the boundary. Fixing means either rounding `remaining_budget`/`physical_max_bid` at source or giving the MILP budget constraint an epsilon — found while fixing the uncontested-DROP bug, out of scope for it
