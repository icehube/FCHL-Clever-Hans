...
- [2026-08-13] [grill] templates/partials/team_panel.html:61 — **a failed MILP strips the buy list out of BOT's own panel with nothing in that panel to say why.** Both the Optimal Projected Points headline and every target row are gated on `milp.status == "Optimal"`, so an Infeasible solve removes them: measured 2026-08-13 at **12 of 63 rows and ~5.6KB**, with the word "Optimizer" appearing nowhere inside `#team-panel`. The warning that explains it renders in `#bid-panel`, a different grid column — side by side at 1024px+, but a long scroll away in the 1-col mobile layout, and the operator's own roster is the panel they are looking at when the list vanishes. Deferred, not silent-and-unknown: the rendering is now pinned in both directions by `TestRenderingWhenTheOptimizerFails`, and the fix is a judgment call about a second warning (or moving the badge into a shared shell) that duplicates a message already on screen — worth deciding deliberately rather than in a test-coverage commit
- [2026-08-15] [grill] templates/partials/trade_panel.html:19 — **six form controls have no accessible name at all**, found while confirming that the new Buyout picker did (it does — Chrome reports `combobox 'Check a buyout'`). Measured the same way, via CDP over the whole rendered page rather than by reading markup: the bid panel's price `<input>` (`bid_panel.html:101`), the Trade Evaluator's `give_player` / `source_team` / `receive_player` (`trade_panel.html:19,29,38`) and the Trade Between Teams form's two multi-selects plus `team_b` (`team_panel.html:219,236,248`) all report an empty name with no name source. Note the bid panel's `player` field is only *accidentally* named — Chrome falls back to its `placeholder`, which is the anti-pattern rather than the fix. Deferred for one specific reason, not for cost: **four of the six are the exact markup the tracked "Trade form: bigger, clearer multi-selects" want will rewrite**, and labelling them now guarantees a collision and a second round of the same decision. Do them in that commit. The two that are not — `bid_panel.html:101` and `team_panel.html:236` — are two attributes and could go any time. The draft is one sighted operator on a mouse, so nothing here blocks draft day; this is consistency with a convention the suite already pins in five places, not a live defect
