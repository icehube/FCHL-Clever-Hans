...
| Trade eval via hypothetical MILP | Same optimizer, just run on a cloned state. No new algorithm needed. **Both sides get the same buyout menu**, and the trade must beat the best NO-trade option: until 2026-09-25 only the trade side got buyouts, so dumping one of BOT's own bad contracts read as a trade gain. **The menu is any SET of legal buyouts, chosen by the solver** (`solve_optimal_roster(..., buyout_candidates=)`, opt-in, used only here): scored one at a time, Kyrou for Thrun + Perunovich read DECLINE because no single buyout beat 1450, while both together scored 1461. Four solves plus two stress solves per evaluation, however many contracts are eligible. |
