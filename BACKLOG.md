...
- [2026-08-18] [grill] tests/test_endpoints.py:1853 (_draft_to) — **a shared test helper drafts a hard-coded player name, which CLAUDE.md forbids for exactly the reason it will bite.** `_draft_to` posts `"Artemi Panarin"` and asserts `status_code == 200`, and `/assign` answers **200 with a toast** when it rejects — so the day `players.csv` no longer carries that name, every test built on this helper passes against a pick that never happened, and the damage surfaces somewhere else entirely (the 2026-08-07 drill saw one missing name arrive as `assert 24 == 25` three tests downstream). Four literal names appear ~39 times across seven test files; `tests/helpers.assign` already fails at the pick and `pool_top`/`a_roster_player` already derive names by role, so the fix is mechanical rather than novel. Deferred: it is a sweep across seven files with no behaviour change, and the next `players.csv` refresh is the moment it pays for itself — do it then, with the fingerprint drill, when the failures are in front of you
...
- [2026-08-18] [grill] main.py:1416 (solve_standings) — **the two multi-solve endpoints are `async def` doing seconds of synchronous CBC work, so they run ON the event loop and every other request queues behind them.** FastAPI dispatches a plain `def` handler to a threadpool and an `async def` one to the loop, so `/solve-standings` (measured 1262ms, 10 solves, 2026-08-17) and `/buyout-indicators` (~15 solves, ~2s) block everything for their duration — including the `/bid-check` that fires while the operator is typing a price, which is the one request a live auction cannot afford to stall. Pre-existing rather than introduced here: `/bid-check` itself is `async def` around ~1000ms of cold solving, so the pattern is the whole file's. The fix is literally deleting `async`, which is what makes it worth NOT doing casually — it moves every touched handler onto a threadpool, and the module globals these endpoints mutate (`exact_projections`, `buyout_indicators`, `market_prices`) are then written off the loop, so the change needs thinking about concurrent mutation rather than just a keyword edit. Deferred with a draft coming; the operator is one person clicking one thing at a time, and `hx-disabled-elt` (2026-08-18) now at least makes the wait visible on the two buttons. Revisit if a draft-day stall is ever actually felt
- [2026-08-06] [grill] main.py:745 (_context) — every endpoint builds the full context (~8.5ms, including a 704-row `bid_limits` list for the available-players table) regardless of how small a fragment it renders. `/bid-check`, `/nominate` and now `/explain?inline=1` reference a handful of its 16 keys and none touches `bid_limits`. `/explain` made this sharper on 2026-08-06: it fires on every bidder toggle and its warm response is ~9ms, essentially all of it this context build for a fragment that uses three keys. Pre-existing — the old whole-panel `auction_control.html` didn't use it either — but the 2026-08-06 panel split made fragments narrower and the waste correspondingly larger. Deferred: small next to the binary search over MILP solves that dominates `/bid-check`, and fixing it properly means a per-panel context builder, which is a cross-endpoint refactor
...
- [2026-08-18] [plan] templates/partials/nomination_panel.html:77 — the RFA and UFA pick cards are ~20 lines of near-identical markup differing only in the heading, the "Secret/Open Bid" label and the prior-team line; a `pick_card(pick, heading, show_prior)` macro would collapse them the way `pick_prices` just collapsed their price line — deferred: folding a refactor that size into the same commit as a feature is what `/grill` exists to catch, and the price-line macro proves the shape first

### test infrastructure

- [2026-08-19] [grill] tests/test_scenarios.py:222 (test_loading_it_twice_gives_the_same_state) — **scenario determinism is only checked within one process, which cannot see the thing the tie-breaks exist for.** All five scenarios load twice and compare; measured 2026-08-19, removing `_fill`'s name tie-break leaves all 58 tests in the file green, because dict iteration is insertion-ordered and both loads see the same order. The tie-breaks in `_fill`/`_drain`/`_reserved_top` matter **across** processes, where hash and set order vary — so a scenario that is stable today could load differently on the draft-day machine and nothing would say so. Deferred rather than papered over: the honest check is a subprocess per scenario with a different `PYTHONHASHSEED` comparing a state digest, which costs an interpreter start (~200ms) per scenario in a file that already runs 6s, and the docstrings now state the limitation instead of implying coverage. Cheapest form if it ever bites: one parametrized test that shells out twice with seeds 0 and 1 for a single scenario, not all five
