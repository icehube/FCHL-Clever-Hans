...
parked on a real event (a no-contract drop mid-auction) and a plan of its own
(the state-dir lock, next). The `players.csv`-refresh class is **empty**: its last entry,
