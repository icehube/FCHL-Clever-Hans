...
- [2026-08-15] [grill] templates/partials/bid_limits.html:21 — **the Available Players NHL column advertises a sort that cannot work.** The header carries `data-sort-col="3"`, `onclick="sortTable(this)"` and `cursor-pointer`, but the cell holds only an `<img>`, and `sortTable` compares `cell.textContent` — measured in Chrome 2026-08-15, that reads `""` for every row, so all keys tie, the stable sort is a no-op and the click leaves the order byte-identical. A control that looks live and is inert is worse than no control: mid-draft you assume the sort took and read the wrong row. Found while building `logs_panel.html`, which had copied the same pattern into two more headers — those are fixed (header made non-sortable) and pinned by `TestTheLogsPanel::test_no_header_offers_a_sort_that_cannot_work`, which states the general rule and so would catch this one too if it were moved into that panel. Deferred here because the honest fix for Available Players is not the same: sorting a 704-row pool by NHL club is plausibly useful, so the fix is to give `sortTable` an `img[alt]` fallback rather than to remove the affordance — a change to shared JS that every sortable table in the app runs, out of scope for a Logs commit
