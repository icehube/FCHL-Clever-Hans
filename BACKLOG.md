...
- [2026-08-07] [owner-testing] trade_panel.html:13 — **the trade form cannot express a legal trade involving a minor-league player.** Both sides of the form read `roster_players`, which excludes the minors: this line for "I Give", and `main.py:1375 (team_players)` for the "I Receive" dropdown. A group 2/3 player in the minors has his salary fully on cap (see the buyout-dots note in CLAUDE.md, which is the same class of bug caught on 2026-08-07) and is perfectly tradeable. `execute_trade` itself is fine — the restriction is in the two lists that feed the form. Deferred: needs a decision on whether minors show inline with a marker or as a separate group, and whether the same widening applies to the buyout dropdown
