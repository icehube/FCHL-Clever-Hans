...
- **Draft actions reset the view to BOT** via `_view_my_team()` — `/assign`, `/undo`, `/buyout`, plus `/reset` and `/load-scenario`, which replace the world. Deliberate, because reading an opponent's Cap Used as yours right after a pick lands is worse than re-opening their roster. On the success path only: a *rejected* assign is not a draft action.
