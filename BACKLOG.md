...
- [2026-08-08] [review] templates/partials/trade_panel.html:111 — **the "(M)" marker in the "I Receive" list has no automated test.** That half of the form is built in JS from `/team-players`, so the marker exists only in the assembled DOM; the endpoint test covers the JSON carrying `is_minor` and the Jinja test covers the "I Give" marker, but deleting the `p.is_minor ? ' (M)' : ''` line leaves the suite green. Verified in Chrome 2026-08-08 (SRL: 23 options, 18 marked, matching the roster) — deferred because a browser case for a two-character suffix is a poor trade against the ~100x slower harness, and the failure mode is a label reading slightly wrong rather than a wrong trade. Fold it into the next browser test that opens the Trade Evaluator for another reason
