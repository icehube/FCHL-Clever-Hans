...
- [2026-09-09] [price-decomposition] `data/model_params.json` (data file, regenerated by the pricer repo — no symbol) — **the F stage-2 points slope is negative above 80 projected points, so the forward price curve peaks at 80 and falls.** `coef_pts_hinge_80` is −0.0504 against a 60–80 slope of +0.0310, giving −0.0194 above the knot: at rank 10 with a $6M lag the curve runs 40pts → $2.51M, 80pts → $6.60M (peak), 132pts → $2.44M — a 132-point forward priced below a 40-point one. Measured 2026-09-09 by `tests/measure_drivers.py`: **259 isolated inversions** among forwards (pairs where more points earns a smaller Points contribution, the other four drivers held out), **zero for D and G**, and zeroing the coefficient raises 8 forwards by **$27.8M** of model price in total. On the live pool it puts Panarin (120pts) at $5.46M against Marner (85pts) at $8.02M. Deferred because **the fix is not in this repo**: `price_model.py` applies the coefficients faithfully — the golden fixture `tests/fixtures/auction_predictions_current.csv` reproduces the same inversion, so it is in the fit the pricer notebook exported, and CLAUDE.md forbids hand-editing the params. Needs a refit there (the piecewise hinge at 80 is over-fitting a thin tail — 8 of 407 forwards clear it). Pinned meanwhile by two `xfail(strict=True)` guards in `tests/test_price_model.py` (`test_the_points_slope_never_goes_negative`, `test_more_points_never_costs_less`), so a corrected export fails the suite as XPASS and forces this entry to be deleted with them. Cost of continuing to defer, measured rather than asserted: every bid limit and every nomination ranking for the league's best forwards is derived from a price that is too low, and the market layer cannot correct it because it only ever caps prices downward.

- [2026-09-10] [grill] `price_model.py:382 (decompose_price)` — **`DriverContribution.floor_logit_delta` is computed for every driver of every decomposition and read by nothing but the test that asserts it reconstructs.** The card shows the stage-2 median only; stage 1 reaches the screen as a single `p_floor` sentence with no attribution. Kept rather than deleted because it is the unfinished half of the feature rather than dead weight: **496 of 705 pool players clamp up to their position's `min_bid`**, so for most of the pool P(floor) *is* the whole price story and the median waterfall explains a figure nobody pays. Deferred because rendering it is not just a new row — stage 1 is **log-odds**, not a multiplier, and its coefficients frequently point the OPPOSITE way from stage 2 (F's `floor_coef_log_rank` is +3.006 against `coef_log_rank` −0.391), so a row reading `×` beside the stage-2 rows would be read as a price factor and be wrong in sign as often as not. It needs its own visual language and its own owner decision, which is a design question rather than a fix. If it is ever decided that stage 1 stays unattributed, delete the field and its test together — a public frozen dataclass carrying an unrendered log-odds number is an invitation to render it as a price.
